...
The image is **preset-agnostic** - `ENVS_PRESET` is NOT baked in. The shared Helm templates
retain runtime preset injection for upstream deployments that consume them:
`deploy/helmfile.yaml.gotmpl` defaults `envsPreset: none`, and
`deploy/values/*/values.yaml.gotmpl` renders it into the container's
`frontend.env.ENVS_PRESET`.

## Adding / removing a preset

1. Edit `registry.json` (the `alias` → URL map).
2. Run `pnpm presets:sync` - regenerates the alias dropdown in `.vscode/tasks.json` (it splices
   only the lines between the `presets:start` / `presets:end` marker comments).
